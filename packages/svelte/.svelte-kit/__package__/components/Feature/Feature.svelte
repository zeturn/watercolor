<script lang="ts">
  let {
    ...
    color = 'default',
    class: className = '',
  }: { icon?: string; title?: string; description?: string; color?: 'default' | 'primary' | 'secondary' | 'success' | 'warning' | 'error' | 'info'; class?: string } = $props()
</script>

<div class="wc-feature wc-feature--{color} {className}">
  {#if icon}<div class="wc-feature__icon">{@html icon}</div>{/if}
  {#if title}<div class="wc-feature__title">{title}</div>{/if}
  {#if description}<div class="wc-feature__description">{description}</div>{/if}
</div>
