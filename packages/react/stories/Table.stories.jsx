...
const StatusPill = ({ tone = 'neutral', children }) => (
  <span className={`wc-table-story-pill wc-table-story-pill--${tone}`}>{children}</span>
)

const ProgressLine = ({ value }) => (
  <div className="wc-table-story-progress" aria-label={`Progress ${value}%`}>
...
export const Primary = {
  ...
  render: (args) => (
    <main className="wc-table-story-page">
      <section className="wc-table-story-shell">
        <div className="wc-table-story-header">
          <p className="wc-table-story-eyebrow">Table</p>
...
export const Dense = {
  ...
  render: (args) => (
    <main className="wc-table-story-page">
      <section className="wc-table-story-shell wc-table-story-shell--compact">
        <div className="wc-table-story-header">
          <p className="wc-table-story-eyebrow">Density</p>
        ...
        <Table {...args}>
          ...
          <TableBody>
            {[1, 2, 3, 4].map((item) => (
              <TableRow key={item}>
                <TableCell size="small">#{item.toString().padStart(3, '0')}</TableCell>
                <TableCell size="small">项目 {item}</TableCell>
                <TableCell size="small"><StatusPill tone="success">活跃</StatusPill></TableCell>
                <TableCell size="small" align="right">
                  <Button size="xs" buttonStyle="default">编辑</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
...
export const StickyHeader = {
  ...
  render: (args) => (
    <main className="wc-table-story-page">
      <section className="wc-table-story-shell">
        <div className="wc-table-story-scroll">
          <Table {...args}>
            <TableHead>
              <TableRow>
...
export const Interactive = () => {
  ...
  const handleSelectAll = () => {
    setSelectedRows((prev) => prev.length === products.length ? [] : products.map((product) => product.id))
  }

  return (
    <main className="wc-table-story-page">
      <section className="wc-table-story-shell">
        <div className="wc-table-story-toolbar">
          <Button size="sm" buttonStyle="default" onClick={handleSelectAll}>
          ...
          <TableHead>
            <TableRow>
              <TableCell component="th" padding="checkbox">
                <input
                  className="wc-table-story-checkbox"
                  type="checkbox"
          ...
          <TableBody>
            {[
              ...
              <TableRow key={row.name}>
                ...
                <TableCell><span className="wc-table-story-rating">{row.level}</span></TableCell>
                <TableCell>
                  <span className="wc-table-story-tags">
                    {row.tags.map((tag) => <StatusPill key={tag}>{tag}</StatusPill>)}
                  </span>
                </TableCell>
              </TableRow>
            ))}
...
export const DataTable = {
  render: () => (
    <main className="wc-table-story-page">
      <section className="wc-table-story-shell wc-table-story-shell--wide">
        <div className="wc-table-story-header">
          <p className="wc-table-story-eyebrow">Data table</p>
          <h1>项目管理看板</h1>
            ...
            {projects.map((project) => (
              <TableRow key={project.id}>
                <TableCell>
                  ...
                  <span className="wc-table-story-meta">ID: {project.id}</span>
                </TableCell>
                <TableCell align="center">
                  <ProgressLine value={project.progress} />
                  <span className="wc-table-story-meta">{project.progress}%</span>
                </TableCell>
                <TableCell>
                  <span>{project.deadline}</span>
                  <span className="wc-table-story-meta">
                    {new Date(project.deadline) < new Date() ? '已过期' : '进行中'}
                  </span>
                </TableCell>
                <TableCell align="center"><StatusPill>👥 {project.team}</StatusPill></TableCell>
                <TableCell align="center">
                  <StatusPill tone={project.priority === 'high' ? 'danger' : project.priority === 'medium' ? 'warning' : 'success'}>
                    {project.priority.toUpperCase()}
                  </StatusPill>
                </TableCell>
                <TableCell align="right">
                  <span className="wc-table-story-actions">
                    <Button size="xs" buttonStyle="default">查看</Button>
                    <Button size="xs" buttonStyle="default">编辑</Button>
                  </span>
                </TableCell>
              </TableRow>
