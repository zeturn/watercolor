            ...
            <section className="wc-recipe-section">
              ...
              <Stack gap="2xs" className="wc-recipe-activity">
                {[['Maya updated the onboarding flow', 'Design system · 12 minutes ago'], ['Alex resolved 8 accessibility issues', 'Website refresh · 1 hour ago'], ['Quarterly research plan was shared', 'Customer insights · Yesterday']].map(([title, copy]) => (
                  <div className="wc-recipe-row" key={title}><span className="wc-recipe-dot" /><div className="wc-recipe-row-main"><div className="wc-recipe-row-title">{title}</div><div className="wc-recipe-row-copy">{copy}</div></div><button className="wc-recipe-icon-button" aria-label={`More actions for ${title}`}><I name="more-horizontal" /></button></div>
                ))}
              </Stack>
            </section>
          </Stack>
