      ...
      <Page as="main" size="full" gutter="none" class="wc-recipe">
        ...
        <Split ratio="sidebar" gap="none" collapse="sm" class="wc-recipe-layout">
          ...
          <Page as="section" size="lg" gutter="lg" class="wc-recipe-main"><Stack gap="2xl" class="wc-recipe-content">
            ...
            <section class="wc-recipe-section"><div class="wc-recipe-section-head"><h2>Recent activity</h2><Button variant="text" size="sm">View all</Button></div><Stack gap="2xs" class="wc-recipe-activity"><div v-for="item in activity" :key="item[0]" class="wc-recipe-row"><span class="wc-recipe-dot" /><div class="wc-recipe-row-main"><div class="wc-recipe-row-title">{{ item[0] }}</div><div class="wc-recipe-row-copy">{{ item[1] }}</div></div><button class="wc-recipe-icon-button"><Icon name="more-horizontal" :size="18" /></button></div></Stack></section>
          </Stack></Page>
        </Split>
      </Page>`
  })
...
export const Settings = {
  render: () => ({
    ...
    template: `
      <Page as="main" size="full" gutter="none" class="wc-recipe">
        <Inline as="header" justify="between" gap="sm" :wrap="false" class="wc-recipe-topbar"><Inline gap="sm" :wrap="false" class="wc-recipe-brand"><span class="wc-recipe-mark">W</span>Workspace settings</Inline><Inline gap="xs" :wrap="false" class="wc-recipe-user"><button class="wc-recipe-icon-button"><Icon name="help-circle" :size="18" /></button><span class="wc-recipe-avatar">HZ</span></Inline></Inline>
...
export const ListDetail = {
  render: () => ({
    ...
    template: `
      <Page as="main" size="full" gutter="none" class="wc-recipe"><Inline as="header" justify="between" gap="sm" :wrap="false" class="wc-recipe-topbar"><Inline gap="sm" :wrap="false" class="wc-recipe-brand"><span class="wc-recipe-mark">W</span>Notes</Inline><Inline gap="xs" :wrap="false" class="wc-recipe-user"><button class="wc-recipe-icon-button"><Icon name="search" :size="18" /></button><span class="wc-recipe-avatar">HZ</span></Inline></Inline>
  ...
  render: () => ({
    ...
    template: `
      <Page as="main" size="md" gutter="md" class="wc-recipe wc-recipe-form-page"><p class="wc-recipe-eyebrow">New workspace</p><h1>Create a project</h1><p class="wc-recipe-lede">Collect just enough information to get the team moving.</p>
        <Stack as="form" gap="2xl" class="wc-recipe-form" @submit.prevent><Stack as="section" gap="md" class="wc-recipe-form-section"><h2>Project details</h2><p>Names and context can be changed later.</p><TextField full-width required label="Project name" placeholder="Website refresh" /><TextField full-width label="Description" multiline :rows="4" placeholder="What does success look like?" /></Stack><Stack as="section" gap="md" class="wc-recipe-form-section"><h2>Ownership</h2><p>Give teammates a clear place to start.</p><div class="wc-recipe-field-grid"><TextField full-width label="Team" model-value="Product design" /><TextField full-width label="Project lead" model-value="Maya Chen" /></div><Inline justify="between" gap="lg" :wrap="false" class="wc-recipe-setting-control"><div><div class="wc-recipe-row-title">Private project</div><div class="wc-recipe-row-copy">Only invited members can access it</div></div><Switch v-model="privateProject" /></Inline></Stack><Inline justify="end" gap="xs" class="wc-recipe-form-actions"><Button type="button" variant="text">Save draft</Button><Button type="submit">Create project</Button></Inline></Stack>
      </Page>`
  })
}
