...
export const Basic = { 
  ...
  render: (args) => ({ 
    components: { FeedVue }, 
    setup() { return { args } }, 
    template: `<div class='p-8 max-w-md'><FeedVue v-bind="args"/></div>` 
  }) 
}
...
export const SocialFeed = {
  ...
  render: (args) => ({
    components: { FeedVue },
    setup() { return { args } },
    template: `<div class='p-8 max-w-lg'><FeedVue v-bind="args"/></div>`
  })
}
...
export const DiscussionThread = {
  ...
  render: (args) => ({
    components: { FeedVue },
    setup() { return { args } },
    template: `<div class='p-8 max-w-2xl'><FeedVue v-bind="args"/></div>`
  })
}
...
export const NewsTimeline = {
  ...
  render: (args) => ({
    components: { FeedVue },
    setup() { return { args } },
    template: `<div class='p-8 max-w-xl'><FeedVue v-bind="args"/></div>`
  })
}
...
export const CompactFeed = {
  ...
  render: (args) => ({
    components: { FeedVue },
    setup() { return { args } },
    template: `<div class='p-4 max-w-sm'><FeedVue v-bind="args"/></div>`
  })
}
...
export const LargeFeed = {
  ...
  render: (args) => ({
    components: { FeedVue },
    setup() { return { args } },
    template: `<div class='p-8 max-w-2xl h-96 overflow-y-auto'><FeedVue v-bind="args"/></div>`
  })
}
...
export const ColorfulFeed = {
  args: { 
    items: [
      {id:1,author:'设计师',time:'1小时前',text:'完成了新的品牌色彩方案',avatar:'https://i.pravatar.cc/40?img=41'},
      {id:2,author:'产品经理',time:'2小时前',text:'用户调研报告已出炉',avatar:'https://i.pravatar.cc/40?img=42'},
      {id:3,author:'开发工程师',time:'3小时前',text:'性能优化提升50%',avatar:'https://i.pravatar.cc/40?img=43'},
    ], 
    showAvatar: true,
    variant: 'timeline',
    color: '#f59e0b', // 橙色主题
...
export const MultiStyleComparison = {
  render: () => ({
    components: { FeedVue },
    setup() { 
      const sampleItems = [
        {id:1,author:'用户A',time:'1小时前',text:'分享了一条动态',avatar:'https://i.pravatar.cc/40?img=51'},
        {id:2,author:'用户B',time:'2小时前',text:'发布了新内容',avatar:'https://i.pravatar.cc/40?img=52'},
      ]
      return { sampleItems } 
    },
    template: `
      <div class='p-8 space-y-8'>
        <div>
          <h3 class='text-lg font-semibold mb-4'>默认样式</h3>
          <div class='max-w-md'>
            <FeedVue :items="sampleItems" variant="timeline" />
          </div>
        </div>
        
        <div>
          <h3 class='text-lg font-semibold mb-4'>粗线条 + 大圆点 (红色)</h3>
          <div class='max-w-md'>
            <FeedVue :items="sampleItems" variant="timeline" color="#ef4444" :line-width="4" :dot-size="18" />
          </div>
        </div>
        
        <div>
          <h3 class='text-lg font-semibold mb-4'>细线条 + 小圆点 (蓝色)</h3>
          <div class='max-w-md'>
            <FeedVue :items="sampleItems" variant="timeline" color="#3b82f6" :line-width="1" :dot-size="6" />
          </div>
        </div>
        
        <div>
          <h3 class='text-lg font-semibold mb-4'>中等样式 (绿色)</h3>
          <div class='max-w-md'>
            <FeedVue :items="sampleItems" variant="timeline" color="#10b981" :line-width="3" :dot-size="12" />
          </div>
        </div>
      </div>
    `
