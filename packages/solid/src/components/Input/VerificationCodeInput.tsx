...
const VerificationCodeInput = ({
  ...
  createEffect(() => {
    ...
    if (typeof value === 'string') {
      value.split('').forEach((ch, idx) => {
        if (inputsRef[idx]) inputsRef[idx].value = ch
      })
    }
  }, [value])
  ...
  const getCurrentValue = () =>
    inputsRef.map((el) => (el ? el.value : '')).join('')
  ...
  const handlePaste = (e) => {
    ...
    text.split('').forEach((ch, i) => {
      if (inputsRef[i]) inputsRef[i].value = ch
    })
  ...
  return (
    <div
      class={['wc-input-code', className].filter(Boolean).join(' ')}
      onPaste={handlePaste}
      {...rest}
    >
