...
const FileInput = ({
  ...
  return (
    ...
    >
      ...
      {variant === 'block' && (
        <>
          <UploadIcon class="wc-file-input__upload-icon" />
          <span class="wc-file-input-content">
            <strong class="wc-file-input-content__title">{label}</strong>
            {description && <span class="wc-file-input-content__subtitle">{description}</span>}
          </span>
        </>
      )}
      {variant === 'button' && <span class="wc-file-button"><span class="wc-file-input__inline-icon" aria-hidden="true">＋</span>{label}</span>}
      {variant === 'icon' && <UploadIcon class="wc-file-icon" />}
    </label>
  )
}
