...
export default function Snackbar({
  ...
  const [visible, setVisible] = createSignal(isControlled ? modelValue : open)
  ...
  const node = (
    <div
      ...
      aria-atomic="true"
    >
      {showIcon && <div class={iconClasses}><StatusIcon severity={severity} /></div>}

      <div class="wc-snackbar__content">
        {title && <div class="wc-snackbar__title">{title}</div>}
        {children || <div class="wc-snackbar__message">{message}</div>}
      </div>

      {(actionLabel || children?.action) && (
        <div class="wc-snackbar__actions">
          {children?.action || (
            <button type="button" class={actionClasses} onClick={handleAction}>
              {actionLabel}
            </button>
          )}
        </div>
      ...
      {showProgress && autoHideDuration > 0 && (
        <div class="wc-snackbar__progress">
          <div class="wc-snackbar__progress-bar" style={{ width: `${progress()}%` }} />
        </div>
      )}
    </div>
  )
