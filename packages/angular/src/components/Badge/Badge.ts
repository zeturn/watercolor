...
@Component({
  ...
  template: `
    <span class="wc-badge wc-badge--{{ color() }} {{ className() }}">
      @if (dot()) {
        <span class="wc-badge__dot"></span>
      }
      <ng-content />@if (display()) {{{ display() }}}
    </span>
  `,
  styles: [
    ':host{display:contents}',
    `.wc-badge{display:inline-flex;align-items:center;gap:4px;min-width:20px;height:20px;padding:0 6px;border-radius:999px;font-size:0.6875rem;font-weight:600;line-height:1;color:#fff;background:var(--wc-text-secondary,#6b7280);box-sizing:border-box}
.wc-badge--primary,
...
.wc-badge__dot{width:6px;height:6px;border-radius:50%;background:currentColor}`,
  ],
