  ...
  template: `
    <div class="wc-banner wc-banner--{{ severity() }} {{ className() }}" role="status">
      <div class="wc-banner__body">
        @if (title()) {
          <div class="wc-banner__title">{{ title() }}</div>
        }
        <div class="wc-banner__content"><ng-content /></div>
      </div>
      @if (closable()) {
        <button class="wc-banner__close" type="button" aria-label="Close" (click)="closed.emit($event)">×</button>
      }
    </div>
  `,
  styles: [
    ':host{display:contents}',
    `.wc-banner{display:flex;align-items:flex-start;gap:12px;width:100%;padding:10px 16px;border:1px solid var(--wc-border-default,rgba(0,0,0,0.08));box-sizing:border-box;color:var(--wc-text-primary,#1a1a1a)}
...
.wc-banner__close{flex:0 0 auto;border:0;background:transparent;color:inherit;font-size:1.25rem;line-height:1;cursor:pointer;padding:0 4px;opacity:0.7}
