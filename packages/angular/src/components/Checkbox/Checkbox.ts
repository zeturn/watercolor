...
@Component({
  ...
  template: `
    <label class="wc-checkbox wc-checkbox--{{ color() }} {{ disabled() ? 'wc-checkbox--disabled' : '' }} {{ className() }}">
      ...
      @if (label()) {
        <span class="wc-checkbox__label">{{ label() }}</span>
      }
    </label>
  `,
  styles: [
    ':host{display:contents}',
    `.wc-checkbox{display:inline-flex;align-items:center;gap:8px;cursor:pointer;color:var(--wc-text-primary,#1a1a1a);font-size:0.875rem}
.wc-checkbox--disabled{cursor:not-allowed;opacity:0.6}
.wc-checkbox__input{position:absolute;opacity:0;width:0;height:0}
.wc-checkbox__box{display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;border:2px solid var(--wc-border-strong,rgba(0,0,0,0.3));border-radius:6px;background:var(--wc-surface-canvas,#fff);color:#fff;box-sizing:border-box;transition:background 0.15s ease,border-color 0.15s ease}
.wc-checkbox__input:checked + .wc-checkbox__box,
.wc-checkbox__input:indeterminate + .wc-checkbox__box{background:var(--wc-accent,#3b82f6);border-color:var(--wc-accent,#3b82f6)}
