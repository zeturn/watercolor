          ...
          @for (item of items(); track $index) {
            <li class="wc-breadcrumb__item">
              ...
              @if ($index < items().length - 1) {
                <span class="wc-breadcrumb__sep" aria-hidden="true">/</span>
              }
            </li>
          }
        </ol>
...
.wc-breadcrumb__item{display:flex;align-items:center;gap:6px}
...
export class Breadcrumb {
  readonly items = input<Array<{ label: string; href?: string }>>([])
