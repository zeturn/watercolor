...
- For Angular, use `@zeturn/watercolor-angular` — same export names as the catalog above; standalone components with `wc-*` selectors (e.g. `<wc-button>`), signal `input()` props, two-way binding via `[(value)]`/`[(checked)]` (`model()`), and output events like `(clicked)`/`(changed)`/`(closed)` instead of React callbacks.
