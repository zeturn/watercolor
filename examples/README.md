...
- `angular-ssr`：可运行的 Angular SSR 示例，演示 SSR 主题预绘制与 `@zeturn/watercolor-angular` 组件用法。
...
`next-ssr`、`svelte-ssr` 与 `angular-ssr` 是可运行示例（需先从仓库根目录 `npm install && npm run build` 构建 workspace 包）。`nuxt-ssr` 目前仍是集成片段。完整说明见站点文档的 SSR 与主题章节。
